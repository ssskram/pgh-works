...
export class Timeline extends React.Component<any, any> {
  ...
  public render() {
    const { timeline, projects, phases, subphases, milestones } = this.props;

    const { expected, actual, phase, subphase, milestone } = this.state;
    ...
    if (timeline != "") {
      timeline.split(", ").forEach(projectID => {
        ...
        // get phases
        const pha = phases.filter(
          phase => phase.projectID == project.projectID
        );
        // get subphases
        const subs = subphases.filter(s => s.projectID == project.projectID);
        // get milestones
        const mlston = milestones.filter(
          mi => mi.projectID == project.projectID
        );
          ...
          pha.forEach(se => {
            if (expected) {
              ...
              const expected = {
                ...
                style:
                  "background-color: #DAECFB; border-color: #FF986C; border-width: 2px;"
              };
              index++;
              items.push(expected);
            }
            ...
            if (actual) {
              ...
              if (se.actualStartDate && se.actualEndDate) {
                const actual = {
                  ...
                  style:
                    "background-color: #3473A8; border-color: #FF986C; border-width: 2px; color: #fffcf5;"
                };
                index++;
                items.push(actual);
              }
            ...
            if (m.percentComplete < 100) {
              if (m.dueDate) {
                mi = {
                  ...
                  group: project.projectID,
                  style:
                    "max-width: 250px; background-color: #ECE0FB; border-color: #ECE0FB;"
                };
                index++;
              }
            } else {
          ...
          <div>
            ...
            >
              ...
              >
                ...
                <Checkbox
                  checked={expected}
                  onChange={() => this.setState({ expected: !expected })}
                />
              </span>
              <span
                style={{
              ...
              >
                ...
                <Checkbox
                  checked={actual}
                  onChange={() => this.setState({ actual: !actual })}
                />
              </span>
              <span style={{ border: "2px solid #FF986C", padding: "8px" }}>
                Phase{" "}
                <Checkbox
                  checked={phase}
                  onChange={() => this.setState({ phase: !phase })}
                />
              </span>
              <span
                style={{
              ...
              >
                ...
                <Checkbox
                  checked={subphase}
                  onChange={() => this.setState({ subphase: !subphase })}
                />
              </span>
              <span
                style={{
                  border: "2px solid #ECE0FB",
              ...
              >
                ...
                <Checkbox
                  checked={milestone}
                  onChange={() => this.setState({ milestone: !milestone })}
                />
              </span>
            </div>
