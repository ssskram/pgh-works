...
export class AllAssets extends React.Component<any, any> {
  ...
  setAssets(assets) {
    if (this.state.assetType != "") {
      ...
      var uniqueAssetNames = removeDuplicates(assets, "assetName").filter(
        asset => asset.assetType == this.state.assetType
      );
  ...
  filter(event) {
    ...
    const asset = this.state.assets.filter(
      asset => asset.assetName == event.value
    );
        ...
        <div key={index}>
          <div className="col-xs-12 col-sm-6">
            <div className="panel panel-button">
              <div
                onClick={() => this.getAssetLink(asset)}
              ...
              >
                <div style={padding15}>
                  <div className="col-md-12">
                    ...
                    <h4>
                      <b>{asset.neighborhood}</b>
                    </h4>
                    <h4>{asset.street}</h4>
                    <h4>{asset.misc}</h4>
                    <h4>
                      Reference{asset.countReferences != 1 && "s"}:{" "}
                      <b>{asset.countReferences}</b>
                    </h4>
                  </div>
                </div>
              </div>
    ...
    return (
      <div>
        ...
        {this.props.assets && assetType == "" && (
          ...
          >
            <div>
              <AssetTypeSelection receiveType={this.setAssetType.bind(this)} />
            </div>
          </Modal>
        )}
        <div className="text-center">
          ...
          {assetType != "" && (
            ...
            >
              <span style={{ letterSpacing: "2px", fontSize: "1.2em" }}>
                {assetType}
              </span>
            </button>
          )}
