  ...
  deleteTag() {
    ...
    if (this.props.tag.tagType != "Street") {
      ...
      const allTagsPerStreet = this.props.tags.filter(tag => {
        return (
          tag.parentID == this.props.tag.parentID &&
          tag.taggedAssetName == this.props.tag.taggedAssetName
        );
      });
  ...
  public render() {
    return (
      <div className="col-md-12 text-center">
        ...
        <button onClick={this.deleteTag.bind(this)} className="btn btn-danger">
          Delete
        </button>
      </div>
    );
