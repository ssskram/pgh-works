...
export default class Subphase extends React.Component<any, any> {
  ...
  public render() {
    ...
    const { subphase, canEdit } = this.props;

    let percentRemaining = 100 - subphase.percentComplete - 1;
    const progressBackground = {
      background:
        "linear-gradient(to right, #DAECFB " +
        subphase.percentComplete +
        "%, #fff 1%, #fff " +
        percentRemaining +
        "%)"
    };
    ...
    return (
      <div className="col-md-6">
        <div style={progressBackground} className="panel">
          {canEdit == true && (
            <button
              onClick={this.setDelete.bind(this)}
              className="pull-right delete-btn"
            >
              X
            </button>
          )}
          <div className="panel-body text-center">
            <div className="col-md-12">
              <h3>
                <b>{subphase.subphaseName}</b>
              </h3>
              <h4>
                <i>{subphase.subphaseStatus}</i>
              </h4>
              <h4>
              ...
              <button onClick={this.openModal.bind(this)} className="btn">
