...
export default class ImportShapes extends React.Component<any, any> {
  ...
  render() {
    const { assets, zoom, center, selectedAsset, showInfoWindow } = this.state;

    const { grabby } = this.props;
        ...
        {assets &&
          ...
          assets.map((asset, index) => {
            if (asset.shape) {
              return (
                <div key={index}>
                  <Polygon paths={[asset.shape.points]} />
                </div>
              );
            }
          })}
        ...
        {showInfoWindow == true && (
          ...
          >
            <div className="col-md-12">
              ...
              <button
                onClick={() => this.props.receiveAsset(selectedAsset)}
