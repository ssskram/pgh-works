...
export const actionCreators = {
    ...
    deleteTask: (item): AppThunkAction<any> => (dispatch, getState) => {
        ...
        dispatch({
            type: deleteTask, item
        })
    }
}

export const reducer: Reducer<TaskState> = (state: TaskState, incomingAction: Action) => {
    ...
    switch (action.type) {
        ...
        case updateTask:
            return {
                ...state,
                tasks: state.tasks.map(task => task.taskID === action.item.taskID ? {
                    ...task,
